 ...
 * Thanks Ian Bernatcki
 */
 const fs = require('fs')
 const { createCanvas, loadImage } = require('canvas')


console.log('new paletter is coming')

loadImage('assets/bno_bw.png').then(image => {
    console.dir(image)
    ...
    console.log('imgdatalen ' + imageData.data.length/3 + " " + height*width)
    ...
    for (var i = 0; i < imageData.width; i++) {
        for (var j = 0, r, g, b, key; j < imageData.height; j++) {
            ...
            if(r > 250 && g > 250 && b > 250) {
                ...
                lumaContrastArray.push(luma/255)
            }
    ...
    // Проверяем долю белого пространства на изображении (Больше лучше)
    const whiteSpace = white.length/(height*width)*100
    console.log('White space ' + Math.floor(whiteSpace) + "%")    

    let result = hueArray.reduce((sum, current) => sum + current, 0);
    console.log("result " + result/(height*width))

    console.log(lumaContrastArray.length)
    let lumaCoSum = lumaContrastArray.reduce((sum, current) => sum + current, 0);
    let lumaCoAve = lumaCoSum/lumaContrastArray.length
    console.log("lumaCoAve " + lumaCoAve)
    console.log("lumaCoAveFromAllScreen " + lumaCoSum/(height*width))
    console.log("lumaCoMed " + Paletter.median(lumaContrastArray))
    // console.log(lumaContrastArray)

    // console.log(hueArray)

    let lumaMedian = Paletter.median(hueArray)
    console.log('med ' + lumaMedian)
    // colorArray.map(function (item) { return item[0] })
})
...
Paletter.getColors = function (url, callback, colorsQuantity) {
    ...
    this.onImageLoad(url, function() {
        console.log('запустилась onImageLoad')
